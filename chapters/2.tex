...
\begin{description}
    ...
    L'architettura di MASDroid, rappresentata anche nella \Cref{fig:masdroid}, può essere suddivisa in tre componenti principali:
...
\begin{description}
    \item[Supporto nelle fasi iniziali.] L'uso dei LLM è particolarmente utile nelle fasi iniziali dell'analisi, grazie al fatto che fornisce una visione d'insieme a un fronte di un basso costo cognitivo. Questo approccio avvantaggia soprattutto i reverse engineer meno esperti, mentre il beneficio si riduce nel caso di analisi più approfondite.
...
Il paradigma agentico rappresenta dunque una naturale evoluzione dell'impiego degli LLM nella SRE, ma è ancora in una fase iniziale e in rapida evoluzione, come testimonia la crescente attenzione della ricerca verso questi sistemi. La loro efficacia deve ancora essere verificata in maniera estesa in scenari realistici, anche attraverso benchmark capaci di catturare adeguatamente le caratteristiche delle attività tipiche di reverse engineering. Le sfide future non riguarderanno quindi soltanto il miglioramento degli LLM, ma anche la progettazione di agenti robusti in grado di sfruttarne pienamente il potenziale, integrandoli efficacemente con gli strumenti e gli ambienti di analisi, così da garantire autonomia e affidabilità al processo di reverse engineering.
