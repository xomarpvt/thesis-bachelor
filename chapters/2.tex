...
Nel \emph{centralized C\&C model}, il botmaster individua uno o più host che fungono da server C\&C e costituiscono il punto di contatto per i bot compromessi, secondo tre principali configurazioni topologiche. La topologia a \emph{stella singola} prevede un unico server C\&C al quale si collegano tutti i bot, secondo un'organizzazione analoga al modello \emph{client-server} (\Cref{fig:botnet_topologies_a}). La topologia a \emph{stella multi-server} utilizza invece più server C\&C, tra loro interconnessi e tra i quali vengono distribuiti i bot, aumentando ridondanza e scalabilità dell'infrastruttura (\Cref{fig:botnet_topologies_b}). Infine, la topologia \emph{gerarchica} prevede che alcuni bot assumano la funzione di \emph{proxy server}, fungendo da intermediari tra gli altri bot e i server C\&C, riducendo il numero di bot che conosce direttamente l'indirizzo del server C\&C (\Cref{fig:botnet_topologies_c}).
...
\begin{figure}[t]
    ...
    \begin{subfigure}{0.30\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/botnet_topology_a.pdf}
        \caption{Centralizzato -- Stella singola}
        \label{fig:botnet_topologies_a}
    \end{subfigure}
    \hspace{0.10\textwidth}
    \begin{subfigure}{0.30\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/botnet_topology_b.pdf}
        \caption{Centralizzato -- Multi-server}
        \label{fig:botnet_topologies_b}
    \end{subfigure}
    ...
    \begin{subfigure}{0.30\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/botnet_topology_c.pdf}
        \caption{Centralizzato -- Gerarchica}
        \label{fig:botnet_topologies_c}
    \end{subfigure}
    \hspace{0.10\textwidth}
    \begin{subfigure}{0.30\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/botnet_topology_d.pdf}
        \caption{Distribuito -- Casuale}
        \label{fig:botnet_topologies_d}
    \end{subfigure}

    \caption[Modelli e topologie delle botnet]{
        Modelli e topologie delle botnet. Adattata da \cite{hachem2011topology}.
    }
    \label{fig:botnet_topologies}
\end{figure}
...
\begin{description}
    ...
    Lo studio introduce due classi di modelli, impiegate in due framework che seguono approcci complementari:

    \begin{itemize}
        \item \emph{End2end-Decompile} (\Cref{fig:llm_decompile_end}): dopo il disassemblaggio del codice binario in istruzioni assembly x86, il LLM genera direttamente il corrispondente codice C, ottenendo risultati superiori sia al decompilatore di Ghidra sia a modelli general-purpose come GPT-4o.
        \item \emph{Refined-Decompile} (\Cref{fig:llm_decompile_ref}): dopo il disassemblaggio, il codice viene inizialmente decompilato mediante Ghidra; successivamente, il LLM utilizza il codice decompilato come punto di partenza per migliorarne leggibilità e correttezza semantica. L'intervento non si limita alla semplice revisione dei nomi e dei tipi delle variabili, ma mira a una rielaborazione più generale del codice, aumentando la potenziale eseguibilità rispetto alla sola decompilazione automatica.
    \end{itemize}
    ...
    \begin{figure}[t]
        ...
        \begin{subfigure}{0.45\textwidth}
            ...
            \caption{Framework \emph{End2end-Decompile} basato sul LLM \emph{LLM4Decompile-End}}
        ...
        \begin{subfigure}{0.45\textwidth}
            ...
            \caption{Framework \emph{Refined-Decompile} basato sul LLM \emph{LLM4Decompile-Ref}}
