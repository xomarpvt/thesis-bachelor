...
\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{complexity-criteria}
...
\begin{equation*}
    {RC}_{\%} = \frac{N_{renamed\ functions}}{N_{total\ functions}} \cdot 100
...
\begin{equation*}
    {RA}_{\%} = \frac{N_{correctly\ named\ functions}}{N_{renamed\ functions}} \cdot 100
...
\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{reversing-criteria}
...
La valutazione del \capture{} prende in esame l'implementazione prodotta dal \codeagent{} in termini di funzionalità, correttezza e conformità alle specifiche, senza estendersi agli aspetti stilistici del codice. Inizialmente vengono considerati la correttezza sintattica del codice, l'aderenza ai \templates{} forniti e al \schema{}. A questi si aggiungono due ulteriori criteri, a cui è assegnato un peso maggiore in quanto più rappresentativi dell'effettiva funzionalità del modulo. Il primo misura la conformità al protocollo descritto nei \report{}, mentre il secondo valuta la \emph{human independence}, ovvero il grado di autonomia del modulo rispetto alla revisione umana, con punteggi maggiori associati a un minor numero di modifiche necessarie per renderlo operativo.

% Il primo riguarda il rispetto del protocollo descritto nei \report{}, mentre il secondo riguarda la \emph{human independence}, che valuta l'autonomia del modulo rispetto alla revisione umana, assegnando punteggi maggiori al diminuire delle modifiche necessarie per renderlo operativo.
...
\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{module-criteria}
...
Nella tabella \Cref{tab:malware_samples} sono riportate le principali informazioni relative ai campioni selezionati, tra cui i nomi univoci utilizzati per identificarli e gli hash SHA-256 dei file binari. Come anticipato, tutti appartengono alla famiglia Mirai, sono in formato ELF e risultano \emph{statically linked}; i primi due sono not-stripped, mentre gli ultimi tre sono stripped.

\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{malware-samples}
    \caption[Caratteristiche principali dei campioni selezionati]{Caratteristiche principali dei campioni di malware selezionati per la campagna sperimentale.}
...
Sulla base dei criteri riportati nella tabella \Cref{tab:complexity_criteria}, la tabella \Cref{tab:complexity_scores} riporta il \emph{Complexity Score} associata a ciascuno dei campioni scelto. I punteggi totali, espressi su un massimo teorico di $165$, vengono anche normalizzati in termini percentuali.

\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{complexity-scores}
    \caption[\emph{Complexity Score} dei campioni selezionati]{\emph{Complexity Score} dei campioni di malware selezionati per la campagna sperimentale.}
...
La metà superiore della tabella elenca il numero complessivo di token utilizzati dalle attività previste dalle \renaming{} e \reversing{} del \sreagent{} e da quelle del \codeagent{}, oltre al totale. Il conteggio è riferito all'intera esecuzione e non distingue tra token di input, che comprendono prompt iniziali e risposte dei tool, e token di output, che includono reasoning, risposte e tool call.

La metà inferiore mostra invece i tempi di esecuzione corrispondenti alle medesime attività. Rispetto al numero di token, la misura dei tempi è maggiormente soggetta a variabilità e quindi meno precisa, pur fornendo una buona indicazione dell'impegno richiesto dalle diverse attività.

\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{execution-stats}
...
\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{classifier-scores}
...
\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{renaming-scores}
    \caption[Risultati della valutazione della \renaming{}]{Risultati della valutazione della \renaming{}.}
...
\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{reversing-scores}
    \caption[Risultati della valutazione della \reversing{}]{Risultati della valutazione della \reversing{}.}
...
\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{module-scores}
    \caption[Risultati della valutazione dei \capture{}]{Risultati della valutazione dei \capture{}.}
...
A questo punto, per fornire una visione sintetica dei risultati e facilitare il confronto tra i diversi LLM e i campioni, sono stati realizzati i grafici a barre mostrati nella \Cref{fig:bar_plot}, che riportano i punteggi complessivi ottenuti nelle tre valutazioni principali.
...
Per quanto riguarda la valutazione del \capture{}, i \emph{Module Score} risultano complessivamente elevati, grazie al contributo dei punteggi massimi ottenuti per i criteri relativi agli aspetti sintattici, architetturali e di integrazione. Tale risultato è coerente con l'obiettivo del sistema di garantire un'integrazione semplice in altri workflow e con il requisito di limitare le implementazioni dei moduli alle sole funzionalità di infiltrazione e cattura, escludendo quelle di natura offensiva. Rispetto ai \emph{Reversing Score}, non si osservano differenze particolarmente marcate, anche considerando i diversi gradi di successo nell'implementazione delle funzionalità del modulo. Come evidenziato nella \Cref{tab:module_scores}, le differenze osservate tra i moduli dipendono principalmente dal criterio \emph{human\_independence}, che risente maggiormente degli errori accumulati nelle diverse fasi della pipeline.

% Questo è interessante soprattutto perché la \reversing{} è l'attività del sistema più rilevante, sia dal punto di vista computazionale, con il tempo e le risorse richieste, sia perché influenza l'accuratezza dell'intero sistema.
%  Si ipotizza che le due variabili siano caratterizzate da una relazione negativa. Tale relazione può essere lineare oppure, più in generale, monotona, a seconda di come gli incrementi della complessità influenzino la capacità del LLM di svolgere con accuratezza la \reversing{}.

Come anticipato, i dati raccolti consentono di analizzare la relazione tra la complessità del campione, misurata mediante il \emph{Complexity Score}, e il punteggio ottenuto nella valutazione della \reversing{}, espresso dal \emph{Reversing Score}. Si ipotizza che le due variabili siano negativamente associate, ovvero che all'aumentare del \emph{Complexity Score} corrisponda una diminuzione del \emph{Reversing Score}. Tale relazione potrebbe essere lineare oppure, più in generale, monotona, senza assumere una specifica forma funzionale.

% Formalizzazione delle ipotesi: \(H_0 : \rho \geq 0\) \(H_1 : \rho < 0\)
% Livello di significatività: \(\alpha=0.05\)

Per la valutazione dell'ipotesi, l'analisi viene condotta separatamente per ciascun LLM. In primo luogo, viene realizzato un grafico a dispersione, riportato in \Cref{fig:scatter_plot}, al fine di rappresentare graficamente la relazione tra il \emph{Complexity Score} e il \emph{Reversing Score}. Successivamente, per verificare la presenza di una relazione lineare, viene calcolato il coefficiente di correlazione di Pearson, mentre per verificare l'esistenza di una relazione monotona, si utilizza il coefficiente di correlazione di Kendall. Poiché l'ipotesi formulata è direzionale e prevede un'associazione negativa, per entrambi i coefficienti viene testata un'ipotesi alternativa unilaterale, assumendo un livello di significatività pari a $\alpha=0.05$. I valori dei coefficienti e dei relativi \emph{p-value} sono riportati in \Cref{tab:correlation_scores}.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.75\textwidth]{img/scatter_plot.pdf}
    \caption[Relazione tra i punteggi \emph{Complexity Score} e \emph{Reversing Score}]{
        Relazione tra i punteggi \emph{Complexity Score} e \emph{Reversing Score} per i diversi LLM.
    }
...
\begin{table}[t]
    \centering
    \ExecuteMetaData[chapters/TAB.tex]{correlation-scores}
    \caption[Correlazione tra \emph{Complexity Score} e \emph{Reversing Score}]{
        Risultati delle correlazioni di Pearson e Kendall tra \emph{Complexity Score} e \emph{Reversing Score}.
    }
    \label{tab:correlation_scores}
\end{table}

L'osservazione della \Cref{fig:scatter_plot} e, soprattutto, dei risultati presentati nella \Cref{tab:correlation_scores} non fornisce evidenza statistica sufficiente a supportare l'ipotesi dell'esistenza di un'associazione negativa tra \emph{Complexity Score} e \emph{Reversing Score}. Sebbene per DS4FP entrambi i coefficienti assumano valori negativi, i relativi \emph{p-value} non raggiungono la soglia di significatività $\alpha$. Per Q3627, invece, entrambi i coefficienti assumono valori positivi, in contrasto con la direzione prevista dall'ipotesi. Tali risultati devono tuttavia essere interpretati con cautela, considerando la numerosità estremamente ridotta del campione (\(n=5\)), che limita fortemente la potenza statistica dei test e la possibilità di generalizzare le osservazioni.

Nel complesso, i risultati suggeriscono che il \emph{Complexity Score}, considerato isolatamente, non sia sufficiente a spiegare la performance nella \reversing{}. In linea con quanto già emerso dalle precedenti analisi, che hanno evidenziato una certa variabilità tra le diverse esecuzioni del LLM e la presenza di un outlier, il successo nella \reversing{} può essere interpretato come il prodotto dell'interazione tra più fattori, riconducibili al campione considerato, al LLM impiegato e alla singola esecuzione. In questa prospettiva, il \emph{Reversing Score} può essere concettualmente rappresentato come una funzione di questi tre fattori, senza che sia possibile quantificarne separatamente il contributo o valutarne le eventuali interazioni.

\begin{equation*}
    Reversing\ Score = f(\mathrm{Campione}, \mathrm{LLM}, \mathrm{Esecuzione})
    \label{eq:reversing_score}
\end{equation*}

Infine, è opportuno considerare le implicazioni dei risultati ottenuti rispetto al rapporto tra il sistema e l'intervento umano. Come già discusso, il sistema è in grado di operare autonomamente fino alla costruzione di \capture{} compatibili con le interfacce previste e potenzialmente integrabili all'interno di workflow più ampi, senza richiedere interventi durante l'esecuzione. Questo comportamento soddisfa il requisito di indipendenza del sistema dall'umano, pur lasciando spazio a una supervisione utile per monitorare le azioni intraprese e le strategie di analisi adottate.

D'altra parte, l'utilizzo operativo dei \capture{} senza alcuna revisione umana risulta più difficile da ipotizzare, poiché nella maggior parte dei casi è necessario apportare alcune modifiche e correzioni prima del loro impiego. Il sistema appare pertanto maggiormente maturo per scenari in cui gli output prodotti vengano sottoposti a una verifica finale da parte dell'umano, considerando che il contributo richiesto risulta contenuto. Il raggiungimento di una piena autonomia richiederebbe invece ulteriori miglioramenti, soprattutto nella \reversing{}, in termini di affidabilità nell'estrazione delle informazioni e di consistenza tra le diverse esecuzioni.

% Il sistema appare pertanto maggiormente maturo per scenari in cui l'intervento umano sia limitato a una fase finale di verifica e aggiustamento degli output prodotti, considerando che il contributo richiesto risulta contenuto.